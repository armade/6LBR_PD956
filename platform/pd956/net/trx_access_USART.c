...
uint8_t trx_reg_read(uint8_t addr)
{
	...
	usart_spi_write_single(AT86RFX_SPI, addr_val);
	//usart_spi_read_packet(AT86RFX_SPI, &register_value, 1);
	usart_spi_read_single(AT86RFX_SPI, &register_value);
...
void trx_reg_write(uint8_t addr, uint8_t data)
{
	...
	usart_spi_write_single(AT86RFX_SPI, addr);
	//usart_spi_write_packet(AT86RFX_SPI, &data, 1);
	usart_spi_write_single(AT86RFX_SPI, data);
	usart_spi_deselect_device(AT86RFX_SPI, &SPI_AT86RFX_DEVICE);
...
void trx_frame_read(uint8_t *data, uint8_t length)
{
	...
	**/
	...
	usart_spi_write_single(AT86RFX_SPI, temp);
